...
# -----------------------------
a <- 10
q_target <- c(서울 = 2, 강원 = 3, 대전 = 5)
lambda_star <- 60
b <- (lambda_star - a) / (q_target^2)

# -----------------------------
# 2) 그래프 — tick, 글자크기, 보조선 추가
# -----------------------------
par(mfrow = c(1, 3), mar = c(4.2, 5, 5, 1), mgp = c(3,2,0), tcl = 1)

xlim_all <- c(0, 6)
ylim_all <- c(0, 75)
x <- seq(0, xlim_all[2], by = 0.01)
yticks <- seq(0, ylim_all[2], by = 5)
xticks <- seq(1, 6, by = 1)
...
# -----------------------------------------
add_grid <- function() {
  abline(v = xticks, lty = 3, col = "gray70")  # 세로 점선
  abline(h = yticks, lty = 3, col = "gray80")  # 가로 점선
}

# -----------------------------
# 서울
# -----------------------------
plot(x, a + b["서울"] * x^2, type = "l", lwd = 2, col = "blue",
     main = "서울의 한계저감비용곡선",
     xlab = "", ylab = "",
     xlim = xlim_all, ylim = ylim_all,
     xaxs = "i", yaxs = "i", yaxt = "n",
     cex.axis = 3.8, cex.lab = 1.6, cex.main = 5)
axis(2, at = yticks, labels = yticks, cex.axis = 3.8)
add_grid()  # ★ 보조선 추가

# -----------------------------
# 강원
# -----------------------------
plot(x, a + b["강원"] * x^2, type = "l", lwd = 2, col = "forestgreen",
     main = "강원의 한계저감비용곡선",
     xlab = "", ylab = "",
     xlim = xlim_all, ylim = ylim_all,
     xaxs = "i", yaxs = "i", yaxt = "n",
...
# 대전
# -----------------------------
plot(x, a + b["대전"] * x^2, type = "l", lwd = 2, col = "red",
     main = "대전의 한계저감비용곡선",
     xlab = "", ylab = "",
     xlim = xlim_all, ylim = ylim_all,
     xaxs = "i", yaxs = "i", yaxt = "n",
...
# -----------------------------
# 2) 그래프 — 원점 시작 + y축 tick 5단위 + 글자 크기 확대
# -----------------------------
par(mfrow = c(1, 3), mar = c(4.2, 5, 5, 1), mgp = c(3,2,0), tcl = 1)

xlim_all <- c(0, max(q_target) * 1.3)
ylim_all <- c(0, lambda_star * 1.3)
x <- seq(0, xlim_all[2], by = 0.01)
yticks <- seq(0, ylim_all[2], by = 5)
...
plot(x, a + b["서울"] * x^2, type = "l", lwd = 2, col = "blue",
     ...
     xlim = xlim_all, ylim = ylim_all,
     xaxs = "i", yaxs = "i", yaxt = "n",
     cex.axis = 3.8, cex.lab = 1.6, cex.main = 5)  # ★ 제목 크기 키움
axis(2, at = yticks, labels = yticks, cex.axis = 3.8)
